...
> Milestone 7 + email-delivery amendment (September 5, 2026): approved proposals are now emailed to a validated, normalized client email address through a provider-neutral server-only email adapter; the manual copy-link flow is removed from the freelancer UI. The client still opens the emailed secure token into the existing account-free portal to accept or decline; token hashing, expiry, revocation, rotation and atomic acceptance are unchanged. Delivery is persisted with honest Sending/Sent/Failed status; failures preserve the offer and expose Resend/Retry; missing configuration returns `EMAIL_NOT_CONFIGURED` and provider outages a safe retryable error; no fake success. Milestone 7 implements the read-only, project-scoped Project AI Chatbot: application-owned retrieval only, server-validated citations (exact quote substrings, server-generated navigation URLs), one bounded repair, explicit insufficient-evidence and `CONTEXT_TOO_LARGE` handling, a deterministic complete Show All Decisions action, and no database mutations from chat. See `docs/milestone-7.md`. This supersedes older text that the freelancer manually shares proposal links and the placeholder notes that the chatbot "remains Milestone 7". The `abrar-dev`-only branch/push instruction and no-deployment rule stand.
