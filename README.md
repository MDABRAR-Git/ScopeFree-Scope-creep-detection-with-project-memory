...
`src/server/ai/provider.ts` exports the shared `AIProvider.generate` contract. It is guarded by `server-only`; UI and business logic do not import a provider SDK. Scope analysis and the Project AI Chatbot (Milestone 7) both use this factory.
...
Native JSON-schema output is off by default; enable `AI_NATIVE_JSON_SCHEMA=true` only after verifying the configured endpoint/model supports it. The adapter has a configurable 1–90 second per-call deadline (30 seconds by default), explicit cancellation/deadline settlement, redirect rejection, bounded output and safe typed failures. Scope analysis and the Project AI Chatbot each use separate prompts, source retrieval, strict schema/evidence validation and at most one repair through this shared adapter (Milestone 7, below). Missing configuration throws `AI_NOT_CONFIGURED`; no runtime fixtures or substitutes exist. Automated transport tests inject responses in tests only; see the milestone report for observed live results.
...
Open **Project Memory** inside a project to see every finalized client decision and each estimate's current undecided pending offer exactly once. Search covers request text and number, decision text, tasks, explanations, assumptions, agreement terms, client comments and stored tags. Filter by pending, accepted, declined or superseded status; results are complete and have no pagination.

Finalized decisions open a read-only detail showing the exact frozen client offer and price range, saved agreement clauses, client comment, source evidence, supersession links, every offer attempt, original AI output and all saved human revisions in chronological order. Source links are generated by the server from project-owned IDs. A malformed or foreign saved snapshot fails the response instead of disappearing from a filtered list. Project Memory never calls AI and never changes scope or billing. The **Project AI Chatbot** is implemented in Milestone 7 (below).

See [the Milestone 6 implementation and verification record](docs/milestone-6.md).

## Milestone 7 — email delivery and Project AI Chatbot

**Emailed proposals (supersedes manual link sharing).** Approving a saved revision and entering a validated **Client email address** now emails the secure proposal link directly to the client; the manual copy-link control is removed from the freelancer UI. The client opens the emailed link into the same account-free token portal to accept or decline — token hashing, expiry, revocation, rotation and atomic acceptance are unchanged. Email uses a provider-neutral, server-only adapter (`src/server/email/provider.ts`); business logic and UI never import a provider SDK. Configure `EMAIL_PROVIDER=http-json`, `EMAIL_API_URL` (a transactional HTTP JSON endpoint such as `https://api.resend.com/emails`), `EMAIL_API_KEY`, `EMAIL_FROM` (a sender your provider has verified) and optional `EMAIL_REQUEST_TIMEOUT_MS` (1000–90000 ms, default 15000). Missing configuration returns `EMAIL_NOT_CONFIGURED`; provider outages/timeouts return a safe retryable `EMAIL_SEND_FAILED`; there is no fake success and no runtime fixture. Delivery is persisted with an honest status (Sending → Sent on provider confirmation, or Failed with a safe message and **Resend**); the offer is preserved on failure. Send and resend are idempotent; a resend rotates to a currently valid token (revoking the previous one). The raw token is never stored — only its hash — and credentials, tokens and full proposal URLs are never logged. Request History shows the destination address and delivery status.

**Project AI Chatbot.** Each project has a read-only, project-scoped chatbot (nav: **Project AI Chatbot**). `POST /api/projects/:projectId/chat` returns `{answer, citations, insufficientEvidence}` (plus a subset disclosure). Retrieval uses application-owned Prisma queries only over the authenticated owner's project — original baseline clauses, applicable accepted amendments, client requests, immutable saved revisions with deterministic price history, accepted/declined decisions, client comments and supersession relationships — with accepted/declined/pending/expired/revoked/superseded authority preserved (never inferred from recency). The model receives bounded evidence and can only produce an answer and citations; it generates no SQL, runs no tools, and there are no embeddings. Every citation is validated on the server: the source id must exist in this project and the quote must be an exact substring of the saved source; navigation URLs are built from server-owned ids, never trusted from the model. One bounded repair attempt is allowed; unsupported questions return an explicit insufficient-evidence answer, and completeness that exceeds the configured context budget returns `CONTEXT_TOO_LARGE` rather than silently omitting records. Price explanations use deterministic stored calculations and recorded edit reasons (and say so when no reason was recorded). A direct **Show All Decisions** action (`GET /api/projects/:projectId/chat`) lists the complete decision set from the database without AI intent detection, pagination or summarization. Chat never mutates any record and its history need not survive refresh.

See [the Milestone 7 implementation and verification record](docs/milestone-7.md).
...
Client **request** (intake) links are still returned once, are not recoverable from the database, and are shared manually. Proposal links are now emailed to the client (Milestone 7) and are never returned to the freelancer UI. If an operation response is lost, retry safely with the same idempotency key; reusing a key with different inputs returns a conflict. There are no payments or automatic charges.
