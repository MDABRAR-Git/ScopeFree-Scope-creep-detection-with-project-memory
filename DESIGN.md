...
The workspace rail owns top-level Projects navigation. The adjacent project rail owns Overview, Baseline, Requirements, Project Memory, and Project AI Chatbot (enabled in Milestone 7). Project sections are route-backed links with a persistent selected state. The chatbot page reuses the dual-rail workspace layout: a scrollable conversation log of grounded answers with source-linked citations, an empty state, an error-retry state, an insufficient-evidence state, a sticky composer with keyboard submit, and a direct Show All Decisions action; long answers and quotes wrap without horizontal overflow. The approved-offer section replaces the manual copy-link control with a required client-email field and honest Sending / Sent / Failed delivery status plus Resend (Milestone 7 emails the secure proposal link). Both rails provide explicit expand and collapse controls inside the rail itself on desktop and labeled drawer controls on mobile. Breadcrumbs show hierarchy without duplicating a separate back row. Projects, requests, clauses, and memory entries use dense rows with status, primary text, metadata, and actions aligned consistently. Status never relies on color alone.
