...
it("rejects invented IDs/quotes, unmatched evidence, invalid relations and uncertainty without questions", () => {
  for (const mutate of [
    ...
    (o: ReturnType<typeof output>) => { o.tasks[0].classification = "covered"; o.tasks[0].estimatedHours = { minimum: 0, likely: 0, maximum: 0 }; },
...
it("accepts new work with no relevant clause without inventing evidence", () => {
  const t = output().tasks[0]; const o = { schemaVersion: 1, explanation: "New unrelated feature.", tasks: [{ ...t, classification: "out_of_scope", matchedScopeClause: null, sourceEvidence: [] }] };
