...
export async function mutateReview(estimateId: string, action: "save" | "approve" | "reopen", body: unknown) {
  ...
  await database(() => db().$transaction(async tx => {
    ...
    const estimate = await tx.estimate.findUniqueOrThrow({ where: { id: estimateId }, include: { proposal: true, revisions: { orderBy: { revision: "desc" }, take: 1 } } });
    ...
    if (estimate.proposal || estimate.status === "PROPOSED") throw new AppError("ESTIMATE_LOCKED", "This estimate has a proposal. Its saved offer cannot be edited here.", 409);
    ...
    if (action === "save") {
      ...
      const { draft, editReason } = revisionInputSchema.parse(input);
      let saved: ReturnType<typeof pricedSnapshot>;
      try { validateAnalysis(draft.analysis, pinned.sources); saved = pricedSnapshot(draft); }
