...
export default async function ProjectsPage() {
  ...
  return <><div className="breadcrumb">Workspace <span>/</span> Projects</div><div className="page-heading"><div><p className="eyebrow">ROOM FOR YOUR BEST WORK</p><h1>Your projects</h1><p className="muted">Start with clarity. Keep every project grounded in what you agreed.</p></div><span className="count-badge">{projects.length} {projects.length === 1 ? "project" : "projects"}</span></div>
    ...
    <section className="project-section" aria-labelledby="projects-title"><div className="section-heading"><h2 id="projects-title">All projects <span>{projects.length}</span></h2><span className="section-caption">Your work, in one place</span></div>
      {projects.length === 0 ? <div className="empty-state"><div className="empty-art" aria-hidden="true"><FolderOpen size={43} strokeWidth={1.2} /><span><Leaf size={19} /></span></div><h3>A fresh start, a clear scope.</h3><p>Your projects will live here. Give your first project a name<br className="desktop-break" /> above to open its workspace.</p><span className="empty-footnote"><LockKeyhole size={13} aria-hidden="true" /> Private to your workspace</span></div> : <div className="project-grid">{projects.map(project => <Link className="project-card" key={project.id} href={`/projects/${project.id}`}><div className="project-card-top"><span className="folder-tile"><FolderOpen size={24} aria-hidden="true" /></span><ArrowUpRight size={19} aria-hidden="true" /></div><h3>{project.name}</h3><p>Project workspace</p><div className="project-card-footer"><span className="neutral-badge">Getting started</span><span>Created {new Intl.DateTimeFormat("en", { month: "short", day: "numeric", timeZone: "UTC" }).format(project.createdAt)}</span></div></Link>)}</div>}
