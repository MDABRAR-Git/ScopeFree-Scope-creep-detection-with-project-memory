...
export function ProjectMemoryView({ projectId, memory }: { projectId: string; memory: ProjectMemory }) {
  return <section className="memory-page" aria-labelledby="memory-title">
    ...
    <section className="memory-overview" aria-label="Current agreement summary"><div className="memory-scope">
    ...
    </div><div className="memory-counts" aria-label="Memory status counts"><span>Accepted <strong>{memory.summary.accepted}</strong></span><span>Superseded <strong>{memory.summary.superseded}</strong></span><span>Declined <strong>{memory.summary.declined}</strong></span><span>Pending <strong>{memory.summary.pending}</strong></span></div></section>
    <section className="memory-tools" aria-label="Search and filter Project Memory"><form method="get" className="memory-search" role="search" noValidate><label htmlFor="memory-search">Search Project Memory</label><div><Search size={17} aria-hidden="true" /><input id="memory-search" name="q" defaultValue={memory.query.q} maxLength={200} placeholder="Request, decision, task, term, comment or tag" />{memory.query.status !== "ALL" && <input type="hidden" name="status" value={memory.query.status} />}<button className="button button-primary">Search</button></div></form>
    <nav className="memory-filters" aria-label="Filter Project Memory">{memoryStatusSchema.options.map(status => <Link key={status} href={href(projectId, memory.query.q, status)} aria-current={memory.query.status === status ? "page" : undefined}>{status === "ALL" ? "All" : labels[status]}</Link>)}{(memory.query.q || memory.query.status !== "ALL") && <Link className="memory-clear" href={`/projects/${projectId}/memory`}>Clear filters</Link>}</nav></section>
    {memory.rows.length ? <div className="memory-list">{memory.rows.map(row => {
      ...
      return <article key={row.id} className={`memory-card memory-card-${row.status.toLowerCase()}`}><div className="memory-card-top"><div><span className={`memory-status memory-${row.status.toLowerCase()}`}>{labels[row.status]}</span><span className="memory-sequence">Request #{row.requestNumber} · Revision {row.approvedRevision}</span></div><time dateTime={row.occurredAt}>{date(row.occurredAt)}</time></div>
        <div className="memory-card-layout"><div className="memory-card-copy"><h3>{row.title}</h3><p className="source-text memory-request">{row.requestText}</p><div className="memory-effect"><span>{row.changesScope ? "Changes the agreement" : "No agreement change"}</span>{row.availability && <span>{row.availability === "ACTIVE" ? "Available for decision" : row.availability === "EXPIRED" ? "Offer expired" : "Offer is stale"}</span>}{row.clientCommentPresent && <span>Client comment recorded</span>}</div></div>
        <div className="memory-card-bottom"><div>{scenarios.map(s => <span key={s}><small>{s}</small><strong>{formatMoney(row.totalChargePaise[s])}</strong></span>)}</div><Link href={target}>{row.kind === "DECISION" ? "Open decision" : "Open offer review"}<ArrowRight size={15} aria-hidden="true" /></Link></div></div>
      </article>;
