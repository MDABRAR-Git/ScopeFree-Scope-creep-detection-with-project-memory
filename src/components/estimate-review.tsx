...
export function EstimateReview({ estimate, onSaved, children }: { estimate: SavedEstimate; onSaved: (estimate: SavedEstimate) => void; children: ReactNode }) {
  ...
  function taskEditor(task: EditableTask, index: number) {
    ...
    return <fieldset className="intake-panel review-task" key={task.id} id={id(path)} aria-describedby={invalid(path) ? id("validation") : undefined}>
      ...
      {noteFields.map(field => <div className="review-field" key={field}><label htmlFor={id(`${path}.${field}`)}>{label} {noteLabels[field]}</label><textarea {...fieldProps(`${path}.${field}`)} rows={3} value={task[field]} onChange={e => changeTask(index, { [field]: e.target.value })} /><p className="review-help">One item per line; up to 20 items, 2,000 characters each.</p></div>)}
      <div className="review-field"><label htmlFor={id(`${path}.complexity`)}>{label} complexity</label><textarea {...fieldProps(`${path}.complexity`)} rows={2} maxLength={2000} value={task.complexity} onChange={e => changeTask(index, { complexity: e.target.value })} /></div>
      <div className="review-field"><label htmlFor={id(`${path}.explanation`)}>{label} explanation</label><textarea {...fieldProps(`${path}.explanation`)} rows={3} maxLength={2000} value={task.explanation} onChange={e => changeTask(index, { explanation: e.target.value })} /></div>
    </fieldset>;
  }
  ...
  return <form className="estimate-review" noValidate aria-label="Estimate review" aria-busy={!!busy} onSubmit={event => { event.preventDefault(); void perform("review"); }}>
    ...
    <div className="analysis-layout"><div className="analysis-tasks">
      {editing && unlocked ? <fieldset ref={editorRef} className="review-editor" id={id("editor")} disabled={!!busy}><legend className="review-sr-only">Review inputs</legend>
        ...
        <div className="intake-panel"><div className="review-field"><label htmlFor={id("analysis.explanation")}>Overall explanation</label><textarea {...fieldProps("analysis.explanation")} maxLength={2000} rows={3} value={form.explanation} onChange={e => change({ ...form, explanation: e.target.value })} /></div>
          ...
          <div className="review-field"><label htmlFor={id("additionalChargeReason")}>Additional charge reason (client-facing)</label><textarea {...fieldProps("additionalChargeReason")} maxLength={500} rows={2} value={form.chargeReason} onChange={e => change({ ...form, chargeReason: e.target.value })} /><p className="review-help">Required when the additional charge is above ₹0.</p></div>
          <div className="review-field"><label htmlFor={id("editReason")}>Review reason{reasonRequired ? " (required)" : " (optional)"}</label><textarea {...fieldProps("editReason")} maxLength={1000} rows={3} value={editReason} onChange={e => { setEditReason(e.target.value); setFeedback(""); setError(""); }} /><p className="review-help">One recorded reason must explain any classification changes or removed tasks.</p></div>
        </div>
